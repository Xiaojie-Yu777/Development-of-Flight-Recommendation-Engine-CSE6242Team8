        ...
        if int(summary['Expected_Delay']) > 0:
            with st.expander(f"{summary['AIRLINE']} - {summary['FL_DATE']} | Total Duration: {int(summary['AIR_TIME'])} min | Total Price: ${summary['TOTAL_PRICE']:.2f} | Est. Delay: {int(summary['Expected_Delay'])} min | Legs: {summary['NUM_LEGS']} (Click to expand)"):
                ...
                for _, flight in itinerary_details.iterrows():
                    ...
                    st.text(f"Duration: {flight['AIR_TIME']} min, Distance: {flight['DISTANCE']} km")
             ...
             with st.expander(f"{summary['AIRLINE']} - {summary['FL_DATE']} | Total Duration: {int(summary['AIR_TIME'])} min | Total Price: ${summary['TOTAL_PRICE']:.2f} | Ahead of Schedule by: {-int(summary['Expected_Delay'])} min | Legs: {summary['NUM_LEGS']} (Click to expand)"):
                st.image(summary['LOGO_URL'], width=100)  # Adjust width as needed
                itinerary_details = flights_df[flights_df['ITINERARY_ID'] == summary['ITINERARY_ID']]
                for _, flight in itinerary_details.iterrows():
                    st.text(f"Flight Number: {flight['FL_NUMBER']}")
...
def main():
    ...
    with st.sidebar:
        ...
        out_date = st.date_input("Departure Date", 
                                  ...
                                  value=tomorrow,
                                  max_value=datetime.today().date() + timedelta(days=30), help="You can only select a date starting from tomorrow onwards.")
